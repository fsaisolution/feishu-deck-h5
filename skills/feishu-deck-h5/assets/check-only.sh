...
#
# 用法: bash check-only.sh <html-path> [--strict] [--visual] [--gate ingest] [--report PATH]
#
...
if [[ $# -lt 1 ]]; then
  cat <<'EOF' >&2
Usage: bash check-only.sh <html-path> [--strict] [--visual] [--gate ingest] [--report PATH]

Args:
  <html-path>   待检查的 HTML 文件
  --strict      把 warn 升级为 error
  --visual      加跑 Playwright 视觉审计 (要求装好 playwright + chromium)
  --gate ingest 入库门禁模式 (要求 PyYAML)
